...
try {
  ...
  switch (command) {
    ...
    case "evaluate":
      print(
        evaluateAction({
          action: flag("--action") ?? "repository.read",
          risk: parseRisk(flag("--risk")),
          target: flag("--target")
        })
      );
      break;
    ...
    case "plan": {
      ...
      print({
        ...
        inspection,
        proposedActions: inspection.findings.map((finding) => ({
          finding,
    ...
    default:
      process.stderr.write(
        "Usage: miseos-steward <inspect|evaluate|plan|taxonomy> [options]\n"
      );
      process.exitCode = 1;
  }
} catch (error) {
