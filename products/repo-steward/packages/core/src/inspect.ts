...
export function inspectRepository(inputRoot: string): RepositoryInspection {
  ...
  if (existsSync(packageJsonPath)) {
    try {
      ...
      if (packageManager === "unknown") packageManager = packageManagerFromField(parsed.packageManager);
    } catch {
  ...
  if (existsSync(packageJsonPath) && packageManager === "unknown") findings.push("package.json exists without a recognized package manager or lockfile.");
