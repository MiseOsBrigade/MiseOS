...
describe("evaluateAction", () => {
  ...
  it("routes medium risk to a pull request", () => {
    expect(evaluateAction({ action: "repository.file.update", risk: "medium" })).toMatchObject({
      tier: 2,
  ...
  it("requires approval for history rewriting", () => {
    expect(evaluateAction({ action: "git.history.rewrite", risk: "high", rewritesHistory: true })).toMatchObject({
      tier: 3,
